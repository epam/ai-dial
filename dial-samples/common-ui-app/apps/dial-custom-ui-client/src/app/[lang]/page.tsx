...
import { ApplicationRoute } from '@/custom-app-ui/client/src/types/routes';
...
import { useTheme } from '@/custom-app-ui/common/src/context/ThemeContext';
import { applyTheme } from '@/custom-app-ui/common/src/utils/apply-theme';
...
export default function Page() {
  ...
  const { dialUrl, applicationName, setAuthProvider } = useAppContext();
  const searchParams = useSearchParams();
  const { themes, root } = useTheme();
  const [__, setData] = useState<AttachmentData | null>(null);
  const [showLogin, setShowLogin] = useState(false);

  const theme = searchParams.get('theme');
  applyTheme(themes, theme, root);
  ...
  useEffect(() => {
    if (isAuthEnabled) {
      if (isClientSessionValid(session)) {
        setShowLogin(false);
        redirect(ApplicationRoute.Main);
      } else {
        setShowLogin(true);
      }
    }
