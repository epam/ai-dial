...
Routes in DIAL are used for communication with external services through registered endpoints in [DIAL Core](/docs/platform/3.core/0.about-core.md). They act as a bridging mechanism between DIAL Core and external applications, facilitating seamless interactions.
...
| **Weight** | No | Yes | Relative traffic weight for [load balancing](/docs/platform/3.core/5.load-balancer.md) among multiple endpoints. A positive number represents an endpoint capacity; zero or negative disables this endpoint from routing. Higher = more traffic share. Default value: 1. |
