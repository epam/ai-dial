...
**Note**
> Refer to [Access Control](/docs/platform/3.core/2.access-control-intro.md) to learn more about Private and Public logical spaces for object storage in DIAL.
...
The effective authorization rule for any object in a sub-folder includes restrictions from all parent folders up to the root. Refer to [Work with publications](/docs/tutorials/1.developers/1.work-with-resources/0.work-with-publications.md#effective-rules) to learn about effective rules for folders.
...
**Note**
> If a folder has a parent folder, all access rules of the parent also apply to the child. Refer to [Work with publications](/docs/tutorials/1.developers/1.work-with-resources/0.work-with-publications.md#effective-rules) to learn about effective rules.

![Nested And/Or access rule blocks](img/76.png)

**Note**
> Refer to [JWT](/docs/tutorials/2.devops/2.auth-and-access-control/1.jwt.md) and [API Keys](/docs/tutorials/2.devops/2.auth-and-access-control/0.api-keys.md) for how to enable access to DIAL resources. Refer to [Configure IDPs](/docs/tutorials/2.devops/2.auth-and-access-control/2.configure-idps/0.overview.md) for supported identity service provider configurations.
...
2. Define application parameters:
   ...
   | **MCP Endpoint** | No | MCP endpoint DIAL Core uses to communicate with the application. Available when Source Type is **Endpoints**. **Transport**: HTTP by default. **Forward per request key**: Forward a [per-request key](/docs/platform/3.core/3.per-request-keys.md) to allow MCP server access to DIAL storage. **Configuration delivery**: `Header` or `Meta`. |
...
**Note**
> Refer to [Interceptors](/docs/platform/3.core/6.interceptors.md) to learn more. You can define interceptors in [Entities → Interceptors](entities/interceptors.md) before attaching them here.
...
**Note**
> Refer to [Entities → Toolsets](entities/toolsets.md) to learn more about tool set entities. Refer to [Chat User Guide](/docs/tutorials/0.user-guide.md#to-publish-toolset) for how end users can publish tool sets. Refer to [DIAL Core API Publications](https://dialx.ai/dial_api#tag/Publications) for managing publication requests via API.
...
| **Forward per request key** | Yes | Yes | When `true`, a [per-request key](/docs/platform/3.core/3.per-request-keys.md) is forwarded to the tool set endpoint so the tool set can access files in DIAL storage. Not allowed when `authType` is `API_KEY`. |
...
In DIAL, prompts created by users—via the [DIAL Core API](https://dialx.ai/dial_api#tag/Prompts) or [UI](/docs/tutorials/0.user-guide.md#prompts)—are stored in each user's private folder. To share them with other users, prompt owners can publish them, or administrators can manually add them to the Public folder.

**Note**
> Prompts are a protected resource. Refer to [Access Control](/docs/platform/3.core/2.access-control-intro.md) to learn how protected resources are handled in DIAL. Refer to [Chat User Guide](/docs/tutorials/0.user-guide.md#publish-1) for how end users can publish prompts. Refer to [DIAL Core API Publications](https://dialx.ai/dial_api#tag/Publications) for managing publication requests via API.
...
| **Content** | Markdown editor supporting plain text with Markdown formatting, Mustache-style variables `{{variableName}}` for dynamic substitution, and a JSON editor. Refer to [Chat User Guide](/docs/tutorials/0.user-guide.md#variables) to learn more about variables in prompts. |
...
**Note**
> Files are a protected resource. Refer to [Access Control](/docs/platform/3.core/2.access-control-intro.md#acls-for-objects) to learn how protected resources are handled in DIAL.
