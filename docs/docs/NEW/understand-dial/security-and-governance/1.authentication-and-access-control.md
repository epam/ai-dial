...
DIAL controls who can reach the platform and what each caller may do with the resources inside it. This page explains the model behind that control: how callers prove their identity (authentication) and how DIAL decides what they are allowed to access (authorization). It is written for architects and developers who want to understand the model before configuring it. For the exact permission matrices, see the [Access control reference](/docs/NEW/understand-dial/security-and-governance/access-control-reference); for setup, see [Auth and access control](/docs/NEW/operating-dial/auth-and-access-control/index) how-tos.

## Two questions: who are you, and what can you do

Every request to [DIAL Core](/docs/NEW/understand-dial/architecture/architecture-highlights) answers two questions in order. Authentication establishes identity — who is making the request. Authorization then decides access — what that identity may read, write, or run.
...
- **API key** authenticates server-to-server callers that have no user context. API keys are defined in DIAL Core's [dynamic settings](/docs/NEW/operating-dial/auth-and-access-control/api-keys) and associated with roles that govern their access.

A third credential, the [per-request key](/docs/NEW/building-with-dial/working-with-dial-resources/per-request-keys), is not used by external callers. DIAL Core generates it internally for the lifetime of a single request when an application acts on a user's behalf. It carries both the application's and the user's identity, and is invalidated when the request completes.
...
- **Runtime objects** are created through the [Unified API](/docs/NEW/understand-dial/architecture/unified-api-overview), which suits resources that regular users create themselves.
...
- [Access control reference](/docs/NEW/understand-dial/security-and-governance/access-control-reference) — the full permission matrices for every object and subject
- [Collaboration and sharing](/docs/NEW/understand-dial/capabilities/collaboration-and-sharing) — how sharing and publication build on this model
- [Usage limits and cost control](/docs/NEW/understand-dial/security-and-governance/usage-limits-and-cost-control) — the limits that roles also enforce

## Next steps

- [Auth and access control](/docs/NEW/operating-dial/auth-and-access-control/index) — configure authentication and authorization in DIAL Core
- [JWT configuration](/docs/NEW/operating-dial/auth-and-access-control/jwt) — connect an identity provider and map roles
- [API keys](/docs/NEW/operating-dial/auth-and-access-control/api-keys) — create keys and bind them to roles
