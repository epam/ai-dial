...
The overlay layout embeds DIAL Chat inside another application (a website or desktop app) via an iframe. To enable it, set the `IS_IFRAME` environment variable to `true` and configure the allowed iframe origins:
...
chat:
  ...
  environment:
    IS_IFRAME: "true"
    ALLOWED_IFRAME_ORIGINS: "https://your-host-app.example.com"
    ALLOW_OPEN_SIGNIN_PAGE_IN_IFRAME: "true"  # if Chat requires login
    ENABLED_FEATURES: "conversations-section,prompts-section,header,..."
```

| Variable | Description |
|---|---|
| `IS_IFRAME` | Set to `true` to enable overlay mode. |
| `ALLOWED_IFRAME_ORIGINS` | Space-separated list of origins allowed to embed Chat (`frame-ancestors` CSP directive). |
| `ALLOW_OPEN_SIGNIN_PAGE_IN_IFRAME` | Set to `true` if the sign-in page must render within the iframe. |

### Wide overlay

Suitable for demo purposes on websites. Use the full `ENABLED_FEATURES` list for maximum functionality.

![Wide overlay — scheme](img/wide-overlay.svg)

![Wide overlay](img/wide-overlay2.png)

### Narrow overlay

Suitable for desktop applications (for example, Excel) or as a standalone component. Remove `conversations-section` and `prompts-section` from `ENABLED_FEATURES` to fit the narrow viewport.

![Narrow overlay](img/simple-mobile.png)
...
To create a new theme, add a new mode in the mapping schema. The mode defines how interface components are colored using the basic palette. See the [DIAL Chat Themes repository](https://github.com/epam/ai-dial-chat-themes) for the full schema and examples.
