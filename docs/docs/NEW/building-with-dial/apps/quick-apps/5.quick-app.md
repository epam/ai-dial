...
:::note
This page covers the original **Quick App** schema (`$id: https://mydial.epam.com/custom_application_schemas/quickapps`). Both Quick App and Quick App 2.0 are fully supported. For new applications, **[Quick App 2.0](./authoring-guide)** is recommended. See [Upgrade to Quick App 2.0](#upgrade-to-quick-app-20) if you want to migrate an existing app.
:::
...
## Upgrade to Quick App 2.0

If you want to move an existing Quick App to the Quick App 2.0 schema, the following table maps fields between the two versions.
...
| `instructions` | `orchestrator.system_prompt` | Use `CustomSystemPromptConfig` with `type: "custom"`. |
| `starters` | `starters` | Same structure. |
| `document_relative_url` | `contexts` | Use `FileContextConfig` entries. Add a `PredefinedTool` with `template_name: "rag_tool"`. |
| `applications_as_tools` | `tool_sets` (type: `"dial-deployment"`) | Use `DialDeploymentSimpleTool` for direct IDs. Condition matching is not available in QA 2.0 — replace with explicit deployment IDs. |
| `web_api_toolset` (RestApiToolset format) | `tool_sets` (type: `"rest-api"`) | Schema is compatible. Move the toolset into `tool_sets` and change the container field name. |
| `web_api_toolset` (WebApiToolsetInfo format) | `tool_sets` (type: `"rest-api"`) | Needs rewriting to the `RestApiTool` format (see [configuration reference](./tool-sets/reference)). |
| `mcp_toolset` | `tool_sets` (type: `"mcp"`) | Move into `tool_sets` array. Schema is compatible. |
| `client_toolset` | No direct equivalent | Client toolsets are not supported in Quick App 2.0. Functionality must be re-implemented as a DIAL application and referenced as a `dial-deployment` tool. |

**Key architectural change:** Quick App 2.0 uses an orchestrator-based architecture with a dedicated agent loop. The `orchestrator.max_iterations` field (default 15) limits how many tool call cycles can occur per request — adjust if your workflow requires deeper loops.

For a guided walkthrough of Quick App 2.0, start with the [authoring guide](./authoring-guide).
