...
##### Configuration of a local interceptor for an applications in DIAL Core dynamic settings
...
So-called Global interceptors apply to any deployment in DIAL and have the most strict rules. In other words they receive original input first and examine the response last.
...
Application Type interceptors apply to [schema-rich applications](/docs/platform/3.core/7.apps.md#schema-rich-applications). You can specify them in the application root JSON schema in `"dial:applicationTypeInterceptors"`.

##### JSON Schema configuration example:
