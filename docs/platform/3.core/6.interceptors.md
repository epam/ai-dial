...
Global interceptors apply to any deployment in DIAL and tend to have the most strict rules. In other words they receive original input first and examine the response last.
...
```json
{
  "globalInterceptors": ["interceptor-id", "interceptor-id"]
}
```
