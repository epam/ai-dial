...
Quick Apps is a no-code orchestrator conceptually similar to OpenAI's GPTs that simplifies the creation of multi-agent workflows. In Quick Apps, you can use AI agents, MCP toolsets, REST APIs, language models and other tools available in the DIAL ecosystem as building blocks to create applications. For example, you can create an app with a toolset allowing it to call an external API to get a real-time weather forecast for a specific location. Another example is a RAG-like application that can generate responses based on predefined sources.
