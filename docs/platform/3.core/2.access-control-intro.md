...
Resources are stored in a dedicated BLOB storage (or a local file system) and can be accessed through the [DIAL Core API](https://dialx.ai/dial_api).

In DIAL, resources can be located in public or private *space*.

|Folder|Description|Ownership|
|---|---|---|
|public|All published resources get to a public folder. It can include sub-folders for files, conversations, prompts etc.|By default, any user can access a public resource. When making a publication request, you can define rules to limit the access.|
|private|All resources that have not been published. It can include sub-folders for files, conversations, prompts.|By default, only resource owner and system admin can access private resources. Owner can share/publish a resource, providing access to other users.|

## How to Grant Access to Resources

DIAL provides several ways to grant access to resources: 

* **Roles**: You can explicitly add roles to DIAL Core config and assign them to JWT, API keys, routes or toolsets to enable access to specific resources. You can also define limits for roles, such as the number of tokens per minute, day, or month allowing you to control the usage.

    **IMPORTANT**: If roles are not assigned to resources (use `userRoles` property for models, apps, toolsets, routes) in [DIAL Core dynamic settings](https://github.com/epam/ai-dial-core), such resources are available to everyone.
...
* **Sharing**: You can share your resources with other users thus allowing them to access your resources with READ or WRITE permissions. Refer to [Sharing](/docs/tutorials/1.developers/1.work-with-resources/1.sharing.md) to learn more.
* **Publications**: You can publish your resources to the public space, making them accessible to all users. When making a publication request, you can define rules to limit the access. Refer to [Publications](/docs/tutorials/1.developers/1.work-with-resources/0.work-with-publications.md) to learn more.
