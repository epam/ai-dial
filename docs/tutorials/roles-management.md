...
5. Assign roles to AI DIAL models, applications, assistants, addons

AI DIAL Core receives the roles assigned to it through user access tokens (JWTs). These roles can be accessed via the path: `resource_access.<client-id>.roles` where `client-id` is a client registered in KeyCloak realm, 
e.g. `chatbot-ui`. 
...
In **Groups**, do the mapping for each group:
...
3. Select a member you want to assign to the group
...
In this instruction, you will learn how to create roles in MS Azure AD and add them to AI DIAL models, applications, assistants, and addons.

Group management process is consisted of two steps:

1. Create groups in Microsoft Azure Active Directory
2. Include groups into JWT custom claim
3. Assign roles to AI DIAL models, applications, assistants, addons

The roles are provided to AI DIAL Core via user access token(JWT) by MS Azure AD and are available via the path: `groups`.

### Step 1: Create groups in Microsoft Azure AD

1. In **Groups**, you can create required groups
...
5. When customizing the claim **groups**, you can choose the option **sAMAccountName** instead of **Group ID** to have group names instead of group UUIDs in the token

> Refer to [MS Documentation](https://learn.microsoft.com/en-us/entra/identity/hybrid/connect/how-to-connect-fed-group-claims#important-caveats-for-this-functionality) to view the detailed info about configuration group claims for applications.

> **Note**: by default, the claim **groups** is not included into the access token. You should add a **scope** to the authorization request in order to obtain an access token.
...
4. Fill the required field in the form to create a new scope. Make sure **Who can consent?** is set to **Admin and users**.

You should add the **scope** you created above to the authorization request, e.g.:
