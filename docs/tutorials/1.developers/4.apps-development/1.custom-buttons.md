...
DIAL applications can include logic that requires rendering of additional controls (buttons) on the DIAL Chat UI to facilitate a specific interactive experience for chat users during conversations.

![](../img/app-button.png)

## Types of Buttons

DIAL supports several standard types of buttons. Applications can use any or all of them at the same time. These types are defined based on the behavior they enable.
...
DIAL applications use [JSON Schema](https://json-schema.org/) to define custom buttons to be rendered by DIAL Chat on UI. A JSON schema describing the buttons must follow the [JSON meta-schema](/metaschema.json) that defines DIAL buttons.

To enable DIAL Chat to render custom buttons on UI, an application must provide a specific JSON in the response to the chat completion/configuration request from DIAL Chat. Refer to [Buttons Workflow](#buttons-workflow) for details.
