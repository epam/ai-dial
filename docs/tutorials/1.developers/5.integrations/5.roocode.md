...
2. In Roo Code Settings, go to Providers.
   ...
   - Model-specific Settings: - Use custom temperature: Enable if needed. Some models do not support the temperature parameter and may not work if the value is not set to 1. If you encounter errors, set the temperature to 1.
   - Advanced settings: - Rate Limit: Set a minimum time between API requests (recommended: 30s–60s). This helps reduce the chance of receiving HTTP 429 (Too Many Requests) errors.
   - Click Save and Done to apply.

![Configuration](../img/roocode-configuration.png)
