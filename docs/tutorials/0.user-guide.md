...
In DIAL Chat you can have a conversation with agents: [language models](#models) and [applications](#applications). You can access all the available agents in [DIAL Marketplace](#dial-marketplace-home-page) and add them to [your workspace](#my-workspace).
...
In **My workspace**, you can access and manage all agents and toolsets that you have created or added to your workspace on [DIAL Marketplace](#dial-marketplace-home-page). Here you can also access [UI wizards](#application-builder) to add [applications of available types](#application-types) and [toolsets](#toolsets).
...
Quick Apps 2.0 is a composer for building applications from reusable components. It gives possibility to put together new applications from DIAL-native tools (apps and AI models) and external integrations (MCP servers), with any LLM registered in DIAL Core acting as the agent orchestrator.
...
|Icon|No|The icon that will be rendered in the chat UI for this app.|
|Description|No|A short description that will be rendered in the chat UI. **Tip:** Add two line breaks and provide an addition description if needed.|
...
|Document relative URLs|No|If your application includes an instruction in the Toolset to query a document(s) in order to generate a response, use this parameter to select document(s) in the [Attachments Manager](#attachments-manager).|
...
|Agents & Toolsets|No|Add [toolsets](#toolsets), [apps](#applications-1) or [AI models](#models) available to you. **Note**: When using toolsets, make sure to be logged in to use them if authentication is required.|
|Instructions|No|Instructions to the language model.|
|Code Interpreter|No|Enable toggle to use code interpreter agent in your app's workflow. Code Interpreter can execute Python code in a controlled environment, analyze results, and provide feedback or visualizations directly within our conversation.|
...
4. In **Allow access...**, specify access rules for the target folder and the authorised audience. For example Role-Equals-Admin. **Note**: the available roles are defined in each organization individually. Contact your support to find out the rules are applicable in your organization.
...
If you are logged in, the card of the toolset displays the authenticated label: 

![](./img/toolset-logged-in.png)

##### To log in

1. Select a toolset that requires authentication and click **Log in** in the actions menu.
2. Proceed with authentication at the MCP server provider.

> **Note**: A toolset can be published with login credentials. In this case, when you click **Log in**, you will be logged in automatically without being redirected to the MCP server provider.

![](./img/toolset-login.png)

You can also find the **Login** button if you click the toolset card to see the details.
...
|Authentication|No|Select one of the available authentication methods.<br />- **With login**: Select to require to log in to use the toolset.<br />- **With login & config**: Select if MCP server provider requires to provide additional authentication credentials.|
