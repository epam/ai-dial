...
In DIAL Chat you can have a conversation with agents: language model and applications. You can access all the available agents in [DIAL Marketplace](#dial-marketplace) and add them to [My workspace](#my-workspace). Refer to [Conversational Agents](#conversational-agents) to learn more about them.
...
> Refer to [My workspace](#my-workspace) to learn how to add and manage applications in DIAL Chat.

In DIAL, an “application” is any programming logic that conforms to the [Unified API](https://dialx.ai/dial_api) of DIAL or registered custom endpoints and packaged as a ready-to-deploy solution.

DIAL Chat users can engage with applications via standard or fully customized interfaces. Applications can interact via the Unified API, or registered custom endpoints, enabling creation of complex scenarios and utilizing all DIAL features.
...
DIAL platform serves as an **application server**, offering tools to develop, deploy and operate custom applications. Refer to [Application Types](#standard-application-types) to learn more about DIAL applications.

> Refer to [DIAL Application Server](https://epam-rail.com/app-server) to learn more.
...
Application types serve as templates for building customized versions of applications. DIAL offers various standard application types accessible from [My workspace](#my-workspace), allowing you to create new applications based on these templates.

> Refer to [Extensibility](https://epam-rail.com/extension-framework) to learn more.

DIAL has a flexible architecture that supports adding custom application types, enabling implementations that go beyond the standard application types and even implementing a fully custom UI replacing the standard chat interface.
...
In DIAL Marketplace where you can find all [applications](#applications) and [language models](#available-models) available on your DIAL environment. Here, you can also find all applications [published](#publish-app) in your organization.
