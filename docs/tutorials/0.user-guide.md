...
If a toolset was [published with credentials](#to-publish-toolset-with-creds), it may be automatically logged in and display the **ORG CREDS** label. DIAL administrators can also log in with organization credentials to make a toolset available to all users within the organization:
...
2. Click any toolset that requires authentication. Refer to [Toolsets](#authentication) to learn more about toolsets authentication feature.
...
4. On the **Manage credentials** modal:
   ...
   - **Entire organization credentials**: Log in/out using organization credentials - this makes the toolset logged in for all users in the organization
