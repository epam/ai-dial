...
> **Note**: A toolset can be [published with login credentials](#to-publish-toolset-with-creds). In this case, when you click **Log in**, you will be logged in automatically without being redirected to the MCP server provider.
...
|Allowed tools|No|Tools in toolsets are functionalities that can be used to extend the capabilities of your toolset. Refer to the corresponding MCP server provider to see the supported tools.|
