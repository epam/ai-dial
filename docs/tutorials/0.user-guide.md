...
By default, when created, any asset, be it agent, conversation, toolset or prompt, is placed in the DIAL file storage in the private folder of its creator. To enable access to an assets to other users, the creator can publish it.

When an asset is published, it is placed in the Public folder in the file storage where all authorized users can assess it, including from the [DIAL Marketplace](#marketplace). 
