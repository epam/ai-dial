...
* Has `READ` and `WRITE` access to all system resources (conversations, prompts and applications) with the exception of private resources.
...
DIAL includes Admin Panel for administrators. 
...
6. In the review mode, you can use arrows to navigate between conversations/prompts and click **Back to publication request** when finished reviewing. This is disabled for application publication request.
