...
You can publish/unpublish your applications, conversations and prompts, or even a collection of those, making them accessible within your organization and on [DIAL Marketplace](#dial-marketplace). Additionally, you can manage access to the published resources, ensuring that the right information is accessible to the right individuals or teams.
...
2. An admin receives the publication request, reviews it and either approves or declines. Refer to [Process Publish Requests](#process-publish-requests) to learn more.
3. In case the publication request has been approved, the published resources (conversations and prompts) become available for the target audience in the Organization section in the chat application. Published applications can be found in [DIAL Marketplace](#dial-marketplace). If the published conversation or application includes attachments or other resources (e.g. Quick apps can include documents for RAG), they will be available in the [Attachments Manager](#attachments-manager) under the Organization section.
...
DIAL includes Admin Panel for administrators. Refer to [Admin Panel](/docs/tutorials/3.admin/home.md) to see the comprehensive user guide of the application for system administrators.

> Watch a [video demo](/docs/video%20demos/4.dial-admin-panel.md) to see Admin Panel in action.

DIAL Chat has specific sections available only for administrators where they can process publication requests and manage Code apps.

> Watch a [video demo](/docs/video%20demos/1.Chat/3.dial-collaboration.md) to see DIAL Chat Admin Panel in action.
...
In **My workspace**, you can access and manage all conversational agents that you have created or added in [DIAL Marketplace](#dial-marketplace). Here you can also access [UI wizards](#application-builder) to add [applications of available types](#application-types).
...
|Configure toolset|No|Enter a valid Json with a toolset configuration that will be used when making requests. E.g. this toolset can define how to make calls to external API. Refer to [Tutorials](/docs/tutorials/2.devops/0.deployment/3.quick_apps_deployment.md) to see a full configuration example.|
...
3. When you configure all the required parameters, click **Save**. Your Mind Map will appear in [My workspace](#my-workspace). You can now use it, share or publish.
...
2. Once the graph is generated, you can view it in the **Content** tab. In this tab, you can view and test the graph, use toolbar to work with nodes.
