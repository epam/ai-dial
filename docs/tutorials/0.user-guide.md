...
* In My Workspace or Marketplace, select an agent and click **Use ...** button to initiate a conversation. Refer to [Converse](#use) to learn more.
...
In the [DIAL Marketplace](#dial-marketplace-home-page), administrators can deploy and undeploy Code Apps of different users.
...
In the [DIAL Marketplace](#dial-marketplace-home-page) section, you can view all language models available on your DIAL environment. There, you can select models to add them to [My workspace](#my-workspace).
...
> Refer to [App Builder](#application-builder) to learn more.
...
> * Watch [demo video](/docs/video%20demos/2.Applications/mindmap-studio.md) to see the DIAL Mind Map Studio in action.
...
In DIAL Marketplace you can find all agents ([applications](#applications) and [language models](#available-models)) and toolsets available on your DIAL environment. Each user can access only those resources that are available to their user role.
...
From the marketplace section, you can click **Back to Chat** to navigate to the main chat screen.

![](./img/dial-marketplace3.png)

From the marketplace screen, you can navigate to your workspace: 

![](./img/my-workspace-icon.png)

##### Filters & Views

* Use the **Type** filters to display any or all [conversational agents](#conversational-agents) and AI models. **Note**, that all agents are displayed by default - use the Type filter to narrow down the scope of the displayed agents. 
...
In the DIAL Marketplace main screen, you can view details of any agent or toolset and add it to [My workspace](#my-workspace).

To simply add/remove an item to/from My workspace, click **Add/Remove to/from My workspace** label:

![](./img/add-to-workspace.png)

You can also add an agent to My workspace by initiating a conversation with it. For example, to add a model:

1. To select a model on Marketplace.
...
In **My workspace**, you can access and manage all agents and toolsets that you have created or added to your workspace on [DIAL Marketplace](#dial-marketplace-home-page). Here you can also access [UI wizards](#application-builder) to add [applications of available types](#application-types) and toolsets.
...
On Marketplace, you can [add AI models to your workspace](#add-to-my-workspace) for your convenience. 

##### Remove Model from Workspace

You can also remove models from your workspace at any time using the bookmark button the on the model's card: 
...
> * Watch a [video demo](/docs/video%20demos/2.Applications/mindmap-studio.md) describing the Mind Map Studio.

##### To Add Mind Map

> Refer to [Mind Map Studio](#mind-map-studio) for details.

1. In My workspace, click **Add app** and select **Mind Map** to launch the [Mind Map Studio](#mind-map-studio).
...
2. Follow the steps to configure application. Refer to [Add/Edit application](#addedit-application-form) form to see the description of parameters.
...
|Attachments type|No|Types of attachments allowed for this application. Provide the types according to [MIME standard](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types). E.g.: image/png and click enter. Enter `*/*` to allow all types.|
|Max. attachments number|No|The maximal number of attachments the application is allowed to accept. Skip to apply the max integer number. Enter `0` to disable attachments.|
|Chat completion URL|Yes|A chat completion URL exposed by your application and used by DIAL Core to send chat completion requests.|
...
Applications in the private space are available only to the application author. To enable access for specific users, application author can generate a shareing link.
...
For instance, if you’re working with an application that uses the MCP toolset and it requires authentication, you won’t be able to access it unless you’re logged in. In this case, the app will show an error message and ask you to authenticate before you can continue.

Therefore, make sure you are authenticated with toolsets you are about to use.

If you are not logged in the toolset card displays the LOGGED OUT note: 

![](./img/toolset-logged-out.png)

A toolset that does not require authentication does not display any note on its card: 

![](./img/no-auth-toolset.png)

If you are logged in, the card of the toolset displays the authenticated note: 

![](./img/toolset-logged-in.png)

##### To log in

1. Select a toolset that requires authentication and click **Login with my creds** in the actions menu.
2. Proceed with authentication at the MCP server provider.

> **Note**: A toolset can be published with login credentials. In this case, when you click **Login with my creds**, you will be logged in automatically without being redirected to the MCP server provider.

![](./img/toolset-login.png)

You can also find the **Login with my creds** button if you click the toolset card to see the details.
...
|Icon|No|The icon that will be rendered in the chat UI for this toolset.|
|Description|No|A short description that will be rendered in the chat UI. Add two line breaks and provide an addition description if needed.|
...
|Endpoint|Yes|Root endpoint of the toolset. This allows applications and LLMs to interact with its tools by invoking this endpoint.|
|Transport protocol|Yes|Default HTTP; choose SSE for server-sent events when supported.|
|Authentication|No|Select one of the available authentication methods.|
|Without authentication|No|Choose to create a toolset that does not require authentication.|
|Allowed tools|No|Tools in toolsets are functionalities that can be used to extend the capabilities of your toolset.|

![](./img/toolset-editor.png)

#### Use

You can use available toolsets as tools in Quick Apps 2.0 to perform specific actions.
...
The published toolset can be unpublished by the toolset owner or the DIAL administrator. When unpublished, the toolset stops being available on the Marketplace and is removed from the public folder. 
...
You can remove toolsets from your workspace at any time.
...
Use **Delete** in the toolset's menu to completely delete the selected toolset. Applications that use this toolset will show an error when trying to communicate with it.
...
> * [DIALX website home page](https://dialx.ai/) is build using DIAL Mind Map Studio. It is a great tool to learn more about DIAL in an interactive and angaging manner.
> * Refer to [video demo](/docs/video%20demos/2.Applications/mindmap-studio.md) to see demos of this functionality. 
> * Refer to [video demo](/docs/video%20demos/2.Applications/mindmap-lotr.md) of the visualization of cinematic universe to see a custom solution created using the DIAL Mind Map Studio.

### General Info

Once you launch the Mind Map Studio, on the first screen you will be able to enter general info parameters. This information will be rendered on the application's card in DIAL Marketplace.
...
When all the required fields in the step 1 are populated, click **Next** to proceed with App settings. In the App settings, you define the data source for your application, generate knowledge graph, view, test and edit nodes.

#### Sources

To create a knowledge graph, you need to provide sources of information for it. One of the main features of DIAL Mind Maps is that they offer a fully deterministic way of information discovery entirely based on the provided sources. The knowledge graph itself and the chat-bot will not provide any information that is not a part of the provided sources. 

DIAL Mind Map studio can generate knowledge graphs based on web URLs, PDF files or a combination of them.

In **Sources**, select **Add link** to provide a web page URL or **Upload file** to provide a file as a source and click **Generate Graph**. 

![](./img/mindmap-step2.png)

The source that you have added will appear in the **Sources** section. 

In Sources, you can see, edit, add or remove sources. 

![](./img/mindmap-step2-3.png)

You can also regenerate the graph form scratch with updated sources if needed. 

![](./img/regenerate-graph.png)

In this section, you can export the entire Mind Map in a ZIP archive or upload one to use it in the Mind Map Studio. 

![](./img/export-graph.png)

#### Graph Settings 

Prior to generating a new knowledge graph, you can enable the graph generation mode to define graph and RAG settings. 

> **Note**: These settings can be set only prior to generation the knowledge graph.

* **Graph settings**: These settings allow you to define the AI model and the rules for building your knowledge graph. 
   - **Model**: Select one of the available language models from the dropdown list. This model will be used to generate the knowledge graph based on the provided sources.
   - **Prompt**: Describe the rules for building the knowledge graph, set the main theme and key focus areas, specify style and language,
* **Rag settings**: These settings allow you to define the AI model, main prompt and a safeguarding prompt for the chatbot.
   - **Model**: Select one of the available language models from the dropdown list. This model will be used to generate responses in the chatbot based on the knowledge graph sources.
   - **Prompt**: Define the main prompt that will guide the chatbot's responses. This prompt should align with the theme and purpose of your knowledge graph.
   - **Guardrails Prompt**: Use this prompt to define boundaries and guidelines for the conversation of a user with the chatbot.

![](./img/graph-settings.png)

#### Content

Once the graph is generated, you can preview it and work with its content in the **Content** tab. 

In this tab, you can view and test the graph, use toolbar to work with nodes, and generate or hide connections between nodes.
...
DIAL Mind Map Studio provides a comprehensive set of customization options for your to build knowledge graphs that fully meet your expectations.

In the **Customize** section, you can access a collection of customization options for the knowledge graph and chatbot. 

You can edit styles both on the UI form and JSON editor for your convenience. You can also import and export your themes.

As you experiment with styles, you can preview changes in real time in the Preview screen.
...
![](./img/preview-graph.png)

When done, click **Save**. Your Mind Map application will appear in [My workspace](#my-workspace).

![](./img/mindmap-step3-1.png)

#### Editing and Collaboration

In **My workspace**, you can [use](#use), [edit](#edit), [share](#share-2) or [publish](#publish-2) your Mind Map application.

Also, you can [share your application](#share-2) with other users and grant editing rights to collaborate on the application development.