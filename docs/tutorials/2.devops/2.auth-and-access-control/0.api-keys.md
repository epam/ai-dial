...
API keys are used by DIAL Core for server-to-server authentication and access control.
...
* API keys must be associated with a project and role; otherwise a key is invalid.
...
To be valid, an API key must be associated with at least one role. Roles in DIAL are used to manage access to system resources, impose limits on token usage and more. 

> * Refer to [About Roles](/docs/platform/0.architecture-and-concepts/6.access-control.md#roles) to learn more about roles in DIAL.
> * Refer to [Access Control](/docs/platform/3.core/2.access-control-intro.md) to learn more about access control in DIAL.
> * Refer to [DIAL Core documentation](https://github.com/epam/ai-dial-core/blob/development/docs/dynamic-settings/roles.md) for a comprehensive configuration guidelines.
...
In the previous example, we assigned the `myRole` role for `myApiKey` API key. On this step, we declare the `myRole` role in the `roles` section to make a valid configuration. 
...
Roles in DIAL can be configured to have limits on number of requests to models, number of allowed tokens, cost and sharing limits.
...
In this example, we define the `myRole` role with token usage limits for the `chat-gpt-35-turbo` model.
