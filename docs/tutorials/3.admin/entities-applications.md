...
2. Define application's parameters
    ...
    | **Container** | Conditional | Select one of the [running containers](/docs/tutorials/3.admin/deployments-applications.md). Required if Source Type is **Application Container**. |
...
| **Completion endpoint** | Yes | Chat completion endpoint URL that DIAL Core will invoke for this application to send user messages to receive AI-generated responses. Read-only and determined by the selected Application Runner as a Source Type. |
...
| **Forward auth token** | No | This parameter allows to determine whether to forward an Auth Token to your apps's endpoint. If enabled, HTTP header with authorization token are forwarded to chat completion endpoint. |
