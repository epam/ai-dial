...
| **Maintainer** | No | Email address of the person or team responsible for the application. |
| **Icon** | No | Logo to visually distinguish the app on the UI. |
| **Topics** | No | Topics are semantic labels that you can assign to apps (e.g. "finance", "support") for better navigation on UI. Click to display a list of available topics. <br /> You can add your own custom topics to the list following these rules:<br />- The topic name must not exceed 255 characters.<br />- The topic name must not contain leading or trailing spaces. |
...
| **Completion endpoint** | Yes | Endpoint URL that will be used to process chat completion requests. <br />Read-only and determined by the selected Application Runner if it is selected as a Source Type for the application. |
...
| **Responses Defaults** | No | Default parameters are applied if a request OpenAI Responses API call doesn't contain them. <br /> Available if responses endpoint is supported. |
