...
| **Responses Defaults** | No | Default parameters applied if a request doesn't contain them in an OpenAI Responses API call. Works the same way as Completion Defaults for the chat completions API. <br /> Available if OpenAI Responses API is supported. |
