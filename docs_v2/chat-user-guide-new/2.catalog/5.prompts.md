...
The **Prompts** tab of the [Catalog](./0.index.md) lists the prompts published across your organization and the ones you saved yourself. Prompts are stored in DIAL, so they follow you to any device you sign in from.
...
Use a prompt as a template by adding variables to its instructions: `{{variableName}}`, or `{{variableName|Default Value}}` to give it a default. Variables aren't limited to numbers — for example, `Who played {{character}} in {{movie}}?`

When you use a prompt that has variables, DIAL Chat opens a form asking for a value for each one before it populates the message box. Variables also let you and others re-run the same conversation with different inputs.
