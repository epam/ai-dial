...
- **Files**, including attachments that came along with a shared conversation, appear under **Shared with Me** in the File Manager. See [Files](./3.files.md).
...
**Warning**
> **Publish** submits the request straight away — the form has no second confirmation step, and the folder tree sits above the button, so it's easy to click before you've scrolled down and checked the author and rules. Review both before you click.
...
- [Files](./3.files.md) — the Shared with Me and Organization tabs in the File Manager
