...
1. Open the [Marketplace](../building-with-dial/developer-tools/chat-customization/6.marketplace.md) from the sidebar.
...
DIAL Core reads its configuration from a `config.json` file. In this setup, the Echo application is registered as a [deployment](../understand-dial/security-and-governance/2.access-control-reference.md) that DIAL Core exposes through the [Unified API](../understand-dial/architecture/4.unified-api-overview.md):
