...
DIAL Core and DIAL Chat serve this endpoint without any metrics-exporter variable. For adapters and applications built on the DIAL SDK, set `OTEL_METRICS_EXPORTER=otlp,prometheus` so they serve it too. For the full procedure and the metric-name caveat, see [Configure metrics](../configure-metrics).
...
- [Configure metrics](../configure-metrics) — the Prometheus half in detail
