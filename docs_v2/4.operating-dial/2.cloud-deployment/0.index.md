...
|-------|------|---------------------|
| `dial/dial` | Umbrella chart. Deploys the full stack — DIAL Core, DIAL Chat, themes, Valkey, and model adapters — in one release. | Standard installs. The provider guides in this section use it. |
| `dial/dial-core` | DIAL Core alone — the Unified API server. | When you manage Chat, Valkey, and adapters separately. |
| `dial/dial-admin` | DIAL Admin (panel frontend and backend), including the [Deployment Manager Backend](https://github.com/epam/ai-dial-admin-deployment-manager-backend) that the Admin Panel uses to build and deploy container-based extensions. | When you run the Admin Panel. |
| `dial/dial-extension` | Generic chart for any container-based extension — adapters, Custom Apps, and the Quick Apps (2.0) Backend. | [Custom Apps deployment](custom-apps-deployment), [Quick Apps installation](quick-apps-installation). |
...
DIAL Core treats Valkey as a cache, not a system of record — the object store holds the durable state. This is what makes DIAL Core horizontally scalable: its service instances are stateless.
...
- **`complete`** — a production-leaning configuration with a managed Redis-compatible cache, an identity provider, and multiple model providers.
