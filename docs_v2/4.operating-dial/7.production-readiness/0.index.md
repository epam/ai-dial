...
- **Valkey keeps volatile, in-memory data for fast access.** It is a cache. Losing it costs a warm-up, not your data.

```mermaid
flowchart LR
  Client -->|Unified API| Core[DIAL Core - stateless, N replicas]
  Core -->|permanent| Blob[(Blob storage)]
  Core -->|cache| Valkey[(Valkey - managed)]
...
- [Observability](../6.observability/0.index.md) — you cannot operate what you cannot see; metrics, alerting, and analytics underpin every page here.
