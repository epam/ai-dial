...
This page catalogs the failures DIAL operators encounter most often, the HTTP status codes DIAL Core returns, and the resolution for each. It is for DevOps and platform engineers running a deployment who need to map a symptom to a cause quickly. It assumes familiarity with the [Operating DIAL](./local-setup/0.index.md) sections it links to; it does not re-explain configuration that those pages already document.
...
1. Read the service logs. Every quick-start and deployment page verifies a healthy start by confirming that the `core`, `chat`, and adapter services logged a startup line with no errors. A service that exits immediately or logs a stack trace on boot points at configuration, not load. See [Logging](./observability/3.logging.md) for log format and levels.
...
For the signals to alert on for each of these failure modes, see [Alerting](./observability/4.alerting.md).

## Startup and deployment

| Symptom | Likely cause | Resolution |
|---|---|---|
| A container fails to start with a port-bind error | Ports `8080` (DIAL Core) or `3000` (DIAL Chat) are already in use | Free the ports or remap them. See [Echo application](./local-setup/1.docker-compose-with-application.md) for the local-setup port requirements. |
...
| Core exits on boot with a configuration error | A required `settings.json` section is missing or malformed | Validate against the [Core configuration reference](./configuration/core/settings-json/0.index.md). Storage, Redis, and at least one identity provider are required. |
| Adapter fails to start when file storage is enabled | `DIAL_URL` is unset while `DIAL_USE_FILE_STORAGE` is `True` | Set `DIAL_URL` to the Core URL. See [Adapter configuration](./configuration/4.adapter-configuration.md). |

## Authentication and access

| Symptom | Likely cause | Resolution |
|---|---|---|
| Every request returns 401 | No identity provider is configured, or the API key is unknown | Configure at least one provider with `jwksUrl` or `userInfoEndpoint`. See [Identity Providers](./configuration/core/settings-json/4.identity-providers.md) and [JWT authentication](./auth-and-access-control/2.jwt.md). |
| 401 with a valid-looking key | Both an API key and an `Authorization` header were sent (Core rejects this as 400), or the `Authorization` header is malformed | Send exactly one credential. See [API keys](./auth-and-access-control/1.api-keys.md). |
| 403 on a model or application | The authenticated user lacks role-based access to that deployment | Grant the role. See [Roles and rate limits](./auth-and-access-control/3.roles-and-rate-limits.md). |
...
| Spike in 429 responses | A user exceeded token or request limits per minute, hour, or day | Inspect and adjust limits. A `Retry-After` header indicates the wait. See [Roles and rate limits](./auth-and-access-control/3.roles-and-rate-limits.md). |
...
Sustained 5xx or latency from adapters warrants an alert; see [Metrics and monitoring](./observability/2.metrics-and-monitoring.md) and [Alerting](./observability/4.alerting.md).

## Storage and Redis

| Symptom | Likely cause | Resolution |
|---|---|---|
| File upload or download fails; resource reads error | Blob storage is misconfigured or unreachable | Verify the provider-specific required fields: S3 needs `bucket` and `region`; Azure Blob needs a `container`; GCS needs `bucket` and `projectId`. See [Storage](./configuration/core/settings-json/2.storage.md). |
...
| Sessions drop or rate limits behave erratically | Redis is unreachable | Redis is required for caching, sessions, and rate-limit counters. Provide a single-server or cluster configuration. See [Redis](./configuration/core/settings-json/3.redis.md). |
...
Losing Redis costs a cache warm-up, not data. Losing blob storage loses permanent data — see [Back up and restore DIAL](./production-readiness/4.backup-and-restore.md).
...
- [MCP server integration](../building-with-dial/apps/quick-apps/quick-app-2/tool-sets/2.mcp-server-integration.md) — tool call failures, toolset auth, OAuth redirects.
- [Create a Quick App via the API](../building-with-dial/apps/quick-apps/quick-app-2/create-via-api.md) — request validation errors.
- [Create and configure a Quick App](../building-with-dial/apps/quick-apps/quick-app-original/create-and-configure.md) — configuration errors.

## Next steps

- [Alerting](./observability/4.alerting.md) — turn these failure modes into alerts before they reach users.
- [Production readiness](./production-readiness/0.index.md) — high availability, scaling, and backup that prevent these failures.
- [Core configuration reference](./configuration/core/settings-json/0.index.md) — the settings behind startup, storage, Redis, and identity errors.
