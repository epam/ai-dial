...
| Control | Description |
|---------|-------------|
| **Create** | Create an application with the current runner. **Application**: creates a deployment in [Entities → Applications](2.entities/2.applications.md). **Assets Application**: creates a public-folder application in [Assets → Applications](4.assets.md#applications). |
...
| **MCP Endpoint** | No | Yes | MCP endpoint. **Transport**: HTTP by default. **Forward per request key**: Forward a [per-request key](../3.building-with-dial/7.working-with-dial-resources/5.per-request-keys.md) to the MCP endpoint. **Configuration delivery**: `Header` or `Meta`. |
...
| **Playback support** | No | Enables [Playback](../6.chat-user-guide/1.conversations.md), which lets users simulate a conversation without invoking model responses, for review and analysis. |
...
This tab displays the JSON schema parameters for the application type. It defines [parameters that must be configured](2.entities/2.applications.md#parameters) to create an instance of an application based on this runner.
...
2. In the modal, select one or more interceptors from the list of [available interceptors](2.entities/4.interceptors.md).
...
To add applications (defined in [Entities → Applications](2.entities/2.applications.md)):
...
In the **App Routes** tab, define and manage routes that DIAL Core uses when interacting with applications created from this runner via specified endpoints. Applications created from this runner inherit these routes automatically—their [App Routes section](2.entities/2.applications.md#app-routes) is pre-populated and read-only.
...
Configure the route's identity and request-handling behavior. Configuration is similar to [Entities → Routes](2.entities/5.routes.md).
...
Define route-specific role assignments to control access to each individual route. Configuration is similar to [Entities → Routes](2.entities/5.routes.md). Use **Inherit Application Roles** to apply roles from the application built on this runner.
...
- [Manage Assets](4.assets.md) — place applications, toolsets, prompts, and files in the public folder
- [Manage Entities](2.entities/2.applications.md) — create and configure application deployments that reference these Builders
- [Monitor activity and roll back changes](8.audit/1.activity-and-rollback.md) — track changes to Builders across the global audit log
