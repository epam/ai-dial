...
**Note**
> Refer to [Enable App](../../3.building-with-dial/1.apps/5.custom-apps/5.register-app.md) to learn more about enabling applications in DIAL.
...
2. Define the application's parameters:
    ...
    | **Container** | Conditional | Select one of the [running containers](../5.deployments/2.container-management.md). Available and required if Source Type is **Application Container**. |
...
| **Attachment types** | No | Yes | Defines the [attachment types](../../3.building-with-dial/5.developer-tools/2.chat-customization/1.custom-content.md#attachments) (images, files) this app can accept:<br />**No attachments**: Disables all attachment types.<br />**All attachments types**: Allows all types of file attachments. Optionally specify max number of attachments.<br />**Specific attachments types**: Enables the user to define/select specific [MIME types](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/MIME_types/Common_types). Start typing to see suggestions or use `<type>/<subtype>` format for a manual entry. |
...
**Note**
> Refer to [Toolsets — Tools overview](./3.toolsets.md#tools-overview) to learn more about the purpose and functionality of this section.
...
In the **Roles** tab, you can create and manage roles that have access to the selected application. Roles are defined in the [Access Management](../6.access-management/1.roles.md) section. Here, you can define user groups that can use specific applications and define rate limits for them.
...
| **Actions** | Additional role-specific actions.<br />When **Make available to specific roles** toggle is off — opens the [Roles](../6.access-management/1.roles.md) section in a new tab.<br />When **Make available to specific roles** toggle is on, you can open the [Roles](../6.access-management/1.roles.md) section in a new tab, set **Set unlimited**, [Remove](#remove-role) the role from the list, or **Reset to default limits**. |
...
You can define interceptors in the [Entities → Interceptors](./4.interceptors.md) section to add them to the processing pipeline of DIAL Core.
...
**Tip**
> If you need a new interceptor, first create it under [Entities → Interceptors](./4.interceptors.md) and then revisit this tab to attach it to the application's configuration.
...
**Note**
> Configuration of this tab is similar to routes. See [Routes documentation](./5.routes.md) for more information.
...
**Note**
> This section mimics the functionality available in the global [Dashboard and Usage Logs](../8.audit/2.monitoring-dashboards.md) and [Activity](../8.audit/1.activity-and-rollback.md) sections, but is scoped specifically to the selected application.
...
- [Manage interceptors](./4.interceptors.md) — attach interceptors for content filtering and compliance across your application workflows.
- [Manage roles and access control](../6.access-management/1.roles.md) — define who can access each application and at what rate limits.
