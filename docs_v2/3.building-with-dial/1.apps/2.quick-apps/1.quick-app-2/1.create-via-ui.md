...
- (Optional) A Toolset registered in DIAL Core if you want to add MCP tools.
...
To ground the model's responses in a document stored in DIAL, click **+ Add** and select dial file or upload it. This adds file info to app context and allows orchestrator/tools to request its content.
...
1. Use the **search bar** to find agents or Toolsets by name.
...
Selected agents and Toolsets appear as chips in the Agents & Toolsets section. Click the **X** on a chip to remove it.
...
See [Add tools and agents to a Quick App 2.0](4.working-with-tools-and-agents.md) for a complete walkthrough of all tool types, including REST API and MCP configurations.
...
To let users attach files, specify the allowed file types (e.g., `.pdf`, `.docx`) — or allow all types explicitly with `*/*` — and set a maximum number of attachments per message. Attachments stay disabled until you specify at least one type.
