...
- **DIAL Core URL** — the base URL of your DIAL Core API (e.g., `https://dial.example.com`). If you access DIAL Chat at `https://chat.dial.example.com`, the Core API is typically at `https://dial.example.com` (without the `chat.` subdomain). Ask your DIAL administrator for the exact URL.
...
Custom applications are saved under your personal bucket. Fetch its ID:

```bash
curl "$DIAL_URL/v1/bucket" -H "api-key: $DIAL_API_KEY"
...
```bash
curl -X PUT "$DIAL_URL/v1/applications/$DIAL_BUCKET/research-assistant" \
  ...
  -d '{
    ...
    "applicationProperties": {
      ...
      "conversation_starters": {
        "starters": [
          { "title": "Current weather", "text": "What is the current temperature in Paris?" },
          { "title": "Compare climates", "text": "What is the climate difference between London and Madrid?" }
        ]
      }
    }
  }'
...
- **`orchestrator`** — the primary model (`gpt-4o-mini`) that processes user messages and decides when to call tools. `max_iterations: 10` allows up to 10 orchestrator operations per turn; a single operation may issue several tool calls. (The default is 15.)
...
- **`tool_sets[1]` (summarization)** — a DIAL deployment tool that calls a second model to summarize text. `conversation_mode.resumable: false` (which replaces the deprecated `propagate_history`) means the summarizer only receives the query argument, not the full conversation. The `fallback_configuration` lets the orchestrator handle summarizer failures gracefully.
- **`conversation_starters`** — suggested prompts shown at the start of conversations. (The old top-level `starters` array is deprecated.)
...
To modify the app (e.g., change the system prompt or add another tool), retrieve the current config and PUT the updated version:

```bash
curl "$DIAL_URL/v1/applications/$DIAL_BUCKET/research-assistant" \
  -H "api-key: $DIAL_API_KEY"
```

Edit the JSON response, then PUT it back:
...
- Added a DIAL deployment tool with a dedicated system prompt, conversation mode control, and fallback handling.
...
- [Add tools and agents](4.working-with-tools-and-agents.md) — reference for the available tool set types
