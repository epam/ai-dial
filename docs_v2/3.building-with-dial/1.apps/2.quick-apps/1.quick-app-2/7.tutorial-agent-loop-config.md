...
For the same tutorial using the UI, see [Tutorial: agent loop (UI)](5.tutorial-agent-loop-ui.md). For the API, see [Tutorial: agent loop (API)](6.tutorial-agent-loop-api.md).
    ...
    "research-assistant": {
      ...
      "applicationProperties": {
        ...
        "conversation_starters": {
          "starters": [
            { "title": "Paris temperature", "text": "What is the current temperature in Paris?" },
            { "title": "London vs Madrid", "text": "What is the climate difference between London and Madrid?" }
          ]
        }
      }
    }
...
- **`orchestrator`** — the primary model that processes user messages and decides when to call tools. `max_iterations: 10` allows up to 10 orchestrator operations per turn; a single operation may issue several tool calls (the default is 15).
...
- **`tool_sets[1]` (summarization)** — a DIAL deployment tool calling a second model. `conversation_mode.resumable: false` (which replaces the deprecated `propagate_history`) sends only the `query` argument, not the full conversation. The fallback lets the orchestrator handle failures gracefully.
...
- [Tutorial: agent loop (API)](6.tutorial-agent-loop-api.md) — create the app with a single curl command
