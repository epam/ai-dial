...
This guide explains how to connect tools to a Quick App 2.0 using the `tool_sets` configuration. It covers the available toolset types with copy-pasteable JSON examples.

## Overview of toolset types
...
| `dial-mcp` | MCP Toolset registered in DIAL Core | Managed by DIAL Core |
...
| `predefined` | Named toolset templates | N/A |

Tools can be combined freely. A single Quick App 2.0 can use multiple toolset types simultaneously.
...
**API** — include the `tool_sets` array inside `applicationProperties` in the request body when you [create or update an app via the applications API](2.create-via-api.md).

**config.json** — include the `tool_sets` array inside `applicationProperties` in your application entry in [DIAL Core's config.json](3.create-via-config.md).

**UI wizard** — the [DIAL Chat wizard](1.create-via-ui.md) does not use JSON. Instead, it provides a visual [Agents & Toolsets picker](../../../../chat-user-guide-new/2.catalog/2.agents.md#build-a-quick-app) where you select pre-registered Toolsets and agents from a list. This means:

- **DIAL-registered Toolsets** (`dial-mcp`) and **agents** (`dial-deployment`) can be added through the UI picker *or* via JSON.
- **REST API tools** (`rest-api`) and **direct MCP server connections** (`mcp`) can only be configured via JSON (API or config.json).

To add tools to an existing app via API, retrieve its current config, add entries to the `tool_sets` array in `applicationProperties`, and save the updated config back. See [Create via API](2.create-via-api.md) for the exact requests.

---

## Add DIAL deployments as tools

Use the `dial-deployment` toolset to call other DIAL applications or models from within your Quick App 2.0. This enables multi-agent patterns: one app can delegate subtasks to specialized apps.

### Simple form: `dial-deployment-simple`

The simplest way to add a DIAL deployment as a tool. The orchestrator calls it directly; by default it does not forward the full conversation context.
...
Use the `rest-api` toolset to call external REST endpoints. Authentication is configured at the toolset level and applies to all tools within it.
...
Use the `mcp` toolset to connect directly to an external MCP server by URL.
...
|---|---|
| `streamable_http` | Standard HTTP transport; works for most MCP servers. |
...
Use the `dial-mcp` toolset to reference a Toolset that has been registered in DIAL Core's `config.json`. The connection details (URL, auth) are managed by the administrator — you only need the `dial_id`.

![Quick App 2.0 wizard — Agents & Toolsets section with '+ Add' panel open and DIAL-MCP toolset type highlighted](../img/tools-add-panel.png)
...
- Your organization has centrally registered Toolsets.
- You want to reference the Toolset without embedding connection credentials in the app config.
- The Toolset is shared across multiple apps.

---

## Configure fallback behavior

Add a `fallback_configuration` to any tool or toolset to control what happens when a tool call fails:
...
All toolset types can coexist in the same `tool_sets` array. The model has access to all tools from all sets simultaneously:
...
Once you add a toolset in the wizard, it appears in the **Agents & Toolsets** list.

![Quick App 2.0 wizard — Agents & Toolsets list showing a newly added toolset entry with its name and type visible](../img/tools-toolset-added.png)
...
- [Toolset examples](./9.toolsets/5.examples.md) — 5 complete copy-pasteable configurations
- [Configuration reference](./9.toolsets/4.reference.md) — full schema documentation for all toolset types
- [Define and register a Toolset](./9.toolsets/1.define-and-register.md) — register an MCP server in DIAL Core for use with `dial-mcp`
