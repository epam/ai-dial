...
This guide explains how to connect tools to a Quick App 2.0 using the `tool_sets` configuration. It covers the available tool set types with copy-pasteable JSON examples.
...
**UI wizard** — the [DIAL Chat wizard](1.create-via-ui.md) does not use JSON. Instead, it provides a visual [Agents & Toolsets picker](create-via-ui#agents--toolsets) where you select pre-registered Tool Sets and agents from a list. This means:
...
- [Tool Set examples](./9.tool-sets/5.examples.md) — 5 complete copy-pasteable configurations
- [Configuration reference](./9.tool-sets/4.reference.md) — full schema documentation for all tool set types
- [Define and register a Tool Set](./9.tool-sets/1.define-and-register.md) — register an MCP server in DIAL Core for use with `dial-mcp`
