...
**API** — include the `tool_sets` array inside `applicationProperties` in the request body when you [create](2.create-via-api.md) or update an app via `POST /v1/applications/{appId}`.
...
**UI wizard** — the [DIAL Chat wizard](1.create-via-ui.md) does not use JSON. Instead, it provides a visual Agents & Toolsets picker (part of **Context & Tools** in the [Settings step](1.create-via-ui.md#step-2-settings)) where you select pre-registered Tool Sets and agents from a list. This means:
