...
To create a Quick App 2.0 through the UI or API instead, see [Create in DIAL Chat](1.create-via-ui.md) or [Create via API](2.create-via-api.md).
...
For one-off app creation by individual users, the [UI wizard](1.create-via-ui.md) or [API](2.create-via-api.md) is simpler.
  ...
  "applicationTypeSchemas": [
    {
      ...
      "dial:applicationTypeSchemaEndpoint": "http://<quickapps2-host>/v1/configuration-support/application-schema",
...
>
> `dial:applicationTypeSchemaEndpoint` lets DIAL Core fetch the **live** application schema from the running Quick Apps 2.0 service (recommended for DIAL Core v0.41.0+, so the schema stays in sync automatically). The editor's configuration-support endpoints are exposed through `dial:applicationTypeRoutes` — see the [ai-dial-quickapps-backend](https://github.com/epam/ai-dial-quickapps-backend) configuration for the full routes block.
...
- A top-level `applicationTypeSchemaId` matching the schema `$id`.
...
See [Roles configuration](../../../../4.operating-dial/4.configuration/1.core/2.config-json/4.roles.md) for full details.
  ...
  "applicationTypeSchemas": [
    {
      ...
      "dial:applicationTypeSchemaEndpoint": "http://quickapps:5000/v1/configuration-support/application-schema",
      "dial:applicationTypeAssistantAttachmentsInRequestSupported": true,
      "dial:appendApplicationPropertiesHeader": false,
      "type": "object",
      "properties": {}
...
- [Create via API](2.create-via-api.md) — programmatic creation
- [Applications configuration reference](../../../../4.operating-dial/4.configuration/1.core/2.config-json/2.applications.md) — all config.json application attributes
- [Configuration reference](./9.tool-sets/4.reference.md) — full Quick App 2.0 schema documentation
