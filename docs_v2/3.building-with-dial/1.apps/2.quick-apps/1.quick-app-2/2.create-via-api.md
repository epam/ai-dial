...
To create a Quick App 2.0 using the visual wizard instead, see [Create in DIAL Chat](1.create-via-ui.md). To provision Quick App 2.0 through infrastructure configuration, see [Create via config.json](3.create-via-config.md).
...
Custom applications are stored in your personal bucket, so first look up your bucket ID, then send a `PUT` request to save the app.

Get your bucket ID:

```bash
curl "https://dial.example.com/v1/bucket" -H "api-key: ${DIAL_API_KEY}"
# → {"bucket":"3CcedGxCx23EwiVbVmscVktScRyf46KypuBQ65miviST", ...}
export DIAL_BUCKET="3CcedGxCx23EwiVbVmscVktScRyf46KypuBQ65miviST"
```

**Endpoint:** `PUT /v1/applications/{bucket}/{application-path}` (operation `saveCustomApplication`)

Replace `{application-path}` with a name or folder path for your app within the bucket — lowercase letters, numbers, hyphens, and `/` (e.g., `research-assistant`).
...
```bash
curl -X PUT "https://dial.example.com/v1/applications/${DIAL_BUCKET}/my-assistant" \
...
```bash
curl -X PUT "https://dial.example.com/v1/applications/${DIAL_BUCKET}/weather-assistant" \
  -H "Content-Type: application/json" \
  -H "api-key: ${DIAL_API_KEY}" \
  -d '{
    ...
    "applicationProperties": {
      ...
      "conversation_starters": {
        "starters": [
          { "title": "London weather", "text": "What is the weather in London?" },
          { "title": "Tokyo temperature", "text": "Current temperature in Tokyo?" }
        ]
      }
    }
  }'
...
```bash
curl -X PUT "https://dial.example.com/v1/applications/${DIAL_BUCKET}/mcp-assistant" \
  -H "Content-Type: application/json" \
  -H "api-key: ${DIAL_API_KEY}" \
  -d '{
...
**Endpoint:** `GET /v1/applications/{bucket}/{application-path}` (operation `getCustomApplication`)

```bash
curl "https://dial.example.com/v1/applications/${DIAL_BUCKET}/weather-assistant" \
  -H "api-key: ${DIAL_API_KEY}"
```
...
**Endpoint:** `GET /openai/applications` (operation `getApplications`)

```bash
curl "https://dial.example.com/openai/applications" \
  -H "api-key: ${DIAL_API_KEY}"
```
...
Use the same `PUT /v1/applications/{bucket}/{application-path}` endpoint with the updated configuration. The request replaces the entire application definition.
...
**Endpoint:** `DELETE /v1/applications/{bucket}/{application-path}` (operation `deleteCustomApplication`)

```bash
curl -X DELETE "https://dial.example.com/v1/applications/${DIAL_BUCKET}/my-assistant" \
  -H "api-key: ${DIAL_API_KEY}"
```
...
|---|---|---|
| 400 | Invalid `applicationProperties` — schema validation failed. | Check the JSON against the [configuration reference](./toolsets/reference). Common issues: missing required fields (`orchestrator`, `contexts`, `tool_sets`), invalid `type` values in toolsets. |
...
- [Create in DIAL Chat](1.create-via-ui.md) — visual wizard walkthrough
- [Create via config.json](3.create-via-config.md) — provision through infrastructure configuration
