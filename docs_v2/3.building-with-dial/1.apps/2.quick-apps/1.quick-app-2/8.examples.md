...
This page contains complete, copy-pasteable Quick App 2.0 configurations for common use cases. Each example is a full `applicationProperties` object suitable for the [DIAL Core API](2.create-via-api.md) or the JSON editor in DIAL Chat.
...
```json
{
  ...
  "conversation_starters": {
    "starters": [
      { "title": "RAG vs fine-tuning", "text": "What is the difference between RAG and fine-tuning?" },
      { "title": "MCP servers", "text": "Explain what an MCP server does." },
      { "title": "Prompt caching", "text": "What are the benefits of prompt caching?" }
    ]
  }
}
```
...
An app that grounds responses in your documents with retrieval-augmented generation. RAG is **not** a built-in `contexts` feature — it is the predefined tool `dial_rag`, which you add to a toolset. The RAG tool accepts `query` and `attachment_urls` parameters and retrieves relevant passages before the model answers.
...
```json
{
  "orchestrator": {
    ...
    "system_prompt": {
      "type": "custom",
      "variables": {},
      "content": "You are a document assistant. Use the dial_rag tool to retrieve relevant passages before answering. If the answer is not in the retrieved content, say so explicitly. Quote relevant passages when helpful."
    }
  },
  "contexts": [],
  "tool_sets": [
    {
      "name": "dial-deployment-toolset",
      "type": "dial-deployment",
      "description": "DIAL deployment tools",
      "tools": [
  ...
  "conversation_starters": {
    "starters": [
      { "title": "Onboarding", "text": "What does the document say about onboarding?" },
      { "title": "Key policies", "text": "Summarize the key policies." },
      { "title": "Compliance", "text": "Find references to the compliance requirements." }
    ]
  }
}
```

The `dial_rag` tool indexes and retrieves from the documents users attach to the conversation. To attach a fixed document instead of a user-supplied one, add it to `contexts` as a `type: "file"` entry with its relative DIAL storage `url` — but note that `contexts` supplies static context, it does not perform retrieval.
...
```json
{
  ...
  "conversation_starters": {
    "starters": [
      { "title": "Berlin weather", "text": "What is the weather in Berlin right now?" },
      { "title": "AI regulation news", "text": "Search for recent news about AI regulation in the EU." },
      { "title": "Compare climates", "text": "Compare the climate in Oslo and Madrid." }
    ]
  }
}
```

Replace `"web-search"` with the DIAL ID of a registered Toolset in your instance.
...
```json
{
  ...
  "conversation_starters": {
    "starters": [
      { "title": "Ticket status", "text": "What is the status of ticket TICK-5821?" },
      { "title": "Ticket resolved?", "text": "Has ticket TICK-3099 been resolved?" }
    ]
  }
}
```
...
```json
{
  ...
  "conversation_starters": {
    "starters": [
      { "title": "Python bugs", "text": "Review this Python function for bugs." },
      { "title": "TypeScript types", "text": "Check this TypeScript snippet for type safety issues." },
      { "title": "SQL security", "text": "Is there a security problem in this SQL query?" }
    ]
  }
}
```
...
- [Add tools and agents](4.working-with-tools-and-agents.md) — full how-to for all toolset types
- [Configuration reference](./9.toolsets/4.reference.md) — complete schema documentation
