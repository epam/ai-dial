...
An app that grounds responses in your documents with retrieval-augmented generation. RAG is **not** a built-in `contexts` feature — it is the predefined tool `dial_rag`, which you add to a tool set. The RAG tool accepts `query` and `attachment_urls` parameters and retrieves relevant passages before the model answers.
...
- [Add tools and agents](4.working-with-tools-and-agents.md) — full how-to for all tool set types
- [Configuration reference](./9.tool-sets/4.reference.md) — complete schema documentation
