...
| **MCP Endpoint** | No | Yes | MCP endpoint. **Transport**: HTTP by default. **Forward per request key**: Forward a [per-request key](../building-with-dial/working-with-dial-resources/5.per-request-keys.md) to the MCP endpoint. **Configuration delivery**: `Header` or `Meta`. |
...
| **Playback support** | No | Enables [Playback](../chat-user-guide/1.conversations.md), which lets users simulate a conversation without invoking model responses, for review and analysis. |
...
**Note**
> JSON schema of Application Runners conforms to the main [meta schema](https://github.com/epam/ai-dial-core/blob/development/config/src/main/resources/custom-application-schemas/schema.json). Refer to [Schema-rich Applications](../building-with-dial/apps/0.index.md#schema-based-apps) to learn more.
...
**Note**
> To learn more about interceptors, refer to [Interceptors](../building-with-dial/interceptors/0.index.md).
