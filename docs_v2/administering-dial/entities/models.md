...
**Note**
> Refer to [Supported Models](../../building-with-dial/adapters/2.supported-providers.md) to learn more about the supported AI models.
...
You can use upstream endpoints to provide alternative URLs DIAL Core will use to send chat completions and responses requests. This enables round-robin load balancing or fallback among multiple hosts. Refer to [Load Balancer](../../operating-dial/configuration/7.load-balancer.md) to learn more. For example, you can provide a model's Docker container URI if the model's container is deployed in DIAL. Refer to [Model Servings](../deployments/container-management.md) to learn more about this use case.
...
| **Weight** | Numeric [weight](../../operating-dial/configuration/7.load-balancer.md#weights) for this endpoint in a multi-upstream scenario. Higher = more traffic share. |
...
In the **Features** tab, you can enable, disable, or override optional capabilities for a specific model. Use model features to tailor DIAL Core's [Unified Protocol](../../understand-dial/architecture/4.unified-api-overview.md) behavior — turning features on when your model supports them, or off when it doesn't.
...
Each toggle corresponds to a capability in the [Unified Protocol](../../understand-dial/architecture/4.unified-api-overview.md#chat-completion-features). Enable them only if your model and adapter fully support that feature.
...
| **Cache** | Whether the deployment supports [LLM caching](../../building-with-dial/advanced-topics/1.prompt-caching.md). |
| **Auto caching** | Indicates whether the deployment supports [automatic caching](../../building-with-dial/advanced-topics/1.prompt-caching.md), where possible. |
...
> - Refer to tutorials to learn how to use roles to define access and cost control for [JWT](../../operating-dial/auth-and-access-control/2.jwt.md) and [API keys](../../operating-dial/auth-and-access-control/1.api-keys.md).
