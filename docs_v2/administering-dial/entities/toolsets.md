...
| **Source Type** | Source type of the tool set:<br />- [MCP Container](../deployments/mcp-containers.md): Tool set is based on a running MCP container.<br />- **External Endpoint**: External API endpoint for externally-deployed custom tool sets.<br />- [MCP Registry](https://registry.modelcontextprotocol.io/): MCP server from the registry. |
| **Source** | Identifier of a tool set source:<br />- For the [MCP Container](../deployments/mcp-containers.md) source type, it is a container ID.<br />- For External Endpoint — a URL of the external endpoint.<br />- For MCP Registry — MCP server name based on [MCP Registry](https://registry.modelcontextprotocol.io/). |
...
2. Define parameters:
    ...
    | **Source Type** | Yes | Available source types:<br />- [MCP Container](../deployments/mcp-containers.md): Tool set is based on a running MCP container.<br />- **External Endpoint**: External API endpoint for externally-deployed custom tool sets.<br />- **MCP Registry**: [MCP Registry](https://registry.modelcontextprotocol.io/) for Model Context Protocol (MCP) servers. |
    | **External Endpoint** | Conditional | Toolset API endpoint for MCP calls. Applies for External Endpoint source type. |
    | **Container** | Conditional | Select one of the available and running [MCP containers](../deployments/mcp-containers.md) from the list. Applies for MCP Container source type. |
...
| **Source Type** | Yes | Yes | The source type of the selected tool set:<br />- [MCP Container](../deployments/mcp-containers.md): Tool set is based on a running MCP container.<br />- **External Endpoint**: External API endpoint for externally-deployed custom tool sets.<br />- **MCP Registry**: [MCP Registry](https://registry.modelcontextprotocol.io/) for Model Context Protocol (MCP) servers. |
| **External Endpoint** | Conditional | Yes | Toolset endpoint for MCP calls. Applies for External Endpoint source type. |
| **Container** | Conditional | Yes | MCP server [container ID](../deployments/mcp-containers.md). Applies for MCP Container deployment source type. |
...
| **Max retry attempts** | Yes | Yes | Number of times DIAL Core will [retry](../../operating-dial/configuration/7.load-balancer.md#fallbacks) routing a failed call to a toolset endpoint (due to timeouts or 5xx errors). |
...
If your tool set was created based on an MCP container deployed in DIAL, the content of this screen is inherited from the related [MCP container](../deployments/mcp-containers.md) and displays all the **enabled** tools. Click [Manage tools](#manage-tools) to access, try, and enable all the available tools.
...
You can create and manage roles in the [Access Management](../access-management/roles.md) section.
...
| **Actions** | Additional role-specific actions:<br />When **Make available to specific roles** toggle is off — opens the [Roles](../access-management/roles.md) section in a new tab.<br />When **Make available to specific roles** toggle is on, you can open the [Roles](../access-management/roles.md) section in a new tab or [remove](#remove-a-role) the role from the list. |
...
**Note**
> This section mimics the functionality available in the global [Dashboard and Usage Logs](../audit/monitoring-dashboards.md) and [Activity](../audit/activity-and-rollback.md) sections, but is scoped specifically to the selected tool set.
...
- [Manage MCP containers](../deployments/mcp-containers.md) — deploy and manage the MCP containers that power your tool sets.
- [Manage roles and access control](../access-management/roles.md) — define who can access each tool set.
- [Manage applications](./applications.md) — configure application deployments that use tool sets via MCP.
