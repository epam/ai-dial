...
For the full component diagram and a walkthrough of every DIAL component, see [Architecture highlights](../2.understand-dial/2.architecture/1.architecture-highlights.md). This page focuses on the design decisions and trade-offs an architect needs to weigh.

## The central idea: one API for everything

Every model, application, adapter, and tool in DIAL is reached through a single, OpenAI-compatible interface: the [Unified API](../2.understand-dial/2.architecture/4.unified-api-overview.md). Because all components speak the same protocol, they are interchangeable from a caller's perspective. An application can call a model, another application, or a toolset without bespoke integration for each. This uniformity is the architectural property that everything else builds on.

## Why a headless core matters

[DIAL Core](../2.understand-dial/2.architecture/1.architecture-highlights.md#dial-core-required) is the only required component. It is headless and runs without a UI. Everything else — DIAL Chat, DIAL Admin, adapters, applications — is optional and can be adopted incrementally. A headless core means DIAL can serve a custom application, an embedded chat widget, or a backend service equally well. You are not locked into the bundled UI. Teams that already have a frontend integrate at the API layer; teams that want a product experience adopt DIAL Chat on top.
...
- [Architecture highlights](../2.understand-dial/2.architecture/1.architecture-highlights.md) — the component architecture in depth
- [Application server](../2.understand-dial/2.architecture/3.application-server.md) — how DIAL runs applications
- [Agentic platform](../2.understand-dial/3.capabilities/1.agentic-platform.md) — composing applications into multi-agent workflows
- [Access control](../2.understand-dial/4.security-and-governance/2.access-control-reference.md) — the access model in detail

## Next steps

- [Building with DIAL](../3.building-with-dial/0.index.md) — how teams build applications on the platform
- [Deployment guide](../4.operating-dial/2.cloud-deployment/0.index.md) — configuration and deployment for the components above
