...
└── Observability (OpenTelemetry)
        ...
        └── Toolsets ──► MCP servers and external services

Optional surfaces: DIAL Chat (UI), DIAL Admin (governance), DIAL Overlay (embedding)
```

[DIAL Core](../2.understand-dial/2.architecture/4.unified-api-overview.md) is the only required component. It is headless and runs without a UI. Everything else — DIAL Chat, DIAL Admin, adapters, applications — is optional and can be adopted incrementally.
