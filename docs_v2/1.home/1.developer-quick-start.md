...
1. Open the [Marketplace](../3.building-with-dial/5.developer-tools/2.chat-customization/6.marketplace.md) from the sidebar.
...
DIAL Core reads its configuration from a `config.json` file. In this setup, the Echo application is registered as a [deployment](../2.understand-dial/4.security-and-governance/2.access-control-reference.md) that DIAL Core exposes through the [Unified API](../2.understand-dial/2.architecture/4.unified-api-overview.md):
